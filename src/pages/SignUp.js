...
function SignUp() {
  ...
  const handleCheckUserEmail = () => {
    if (userEmail.length >= 4 && userEmail.length <= 10) {
      setUserEmailMessage('사용할 수 있는 이메일입니다.');
    } else {
      setUserEmailMessage('이미 사용 중인 이메일입니다.');
    }
  };
    ...
    <div>
      ...
      <div className="SignInfo">
        ...
        <span style={{ fontSize: '20px', fontWeight: '400' }}>
          SELECTORS의 더 많은 기능을 사용하기 위해 가입하세요!
        </span>
        <div className="UserInfo">
          ...
          />
          <br />
          <span>비밀번호는 영문, 숫자, 특수문자 포함 6~20자로 구성해주세요!</span>
          <br />
          <br />
          ...
          <br />
          <span>한글, 영문, 숫자 포함 2~10자로 구성해주세요. 띄어쓰기는 불가능합니다!</span>
          <br />
          <br />
          <label htmlFor="userEmail">이메일*</label>
          <span
            className={userEmailMessage === '사용할 수 있는 이메일입니다.' ? 'validInput' : 'invalidInput'}
          >
            {userEmailMessage}
          </span>
          <br />
          ...
          <button className="button doubleCheck" onClick={handleCheckUserEmail}>
            중복확인
          </button>
          <br />
          ...
          <input
            type="text"
            className="vertifyNum"
            placeholder={
              timer > 0 ? `${Math.floor(timer / 60)}분 ${timer % 60}초` : '인증 시간이 만료되었습니다.'
            }
          />
          ...
          <input type="text" className="userBirth" placeholder="YYYYMMDD 형식으로 입력" />
          <br />
